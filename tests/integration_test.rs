...
fn test_by_pinyin_output_format() {
    ...
    assert!(first_line.contains(":"), "First line should contain ':' separator");
    
    // Test: should contain numbers (frequency)
    assert!(first_line.chars().any(|c| c.is_ascii_digit()), "First line should contain frequency count");
}
...
fn test_by_pinyin_fold_option() {
    ...
    assert!(folded_lines >= normal_lines, "Folded output should have equal or more lines");
}
...
fn test_by_tone_output_format() {
    ...
    assert!(stdout.contains("jī") || stdout.contains("jí") || stdout.contains("jǐ") || stdout.contains("jì"), 
